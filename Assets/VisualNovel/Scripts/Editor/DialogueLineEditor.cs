...
public class DialogueLineEditor : EditorWindow
{
    ...
    private void OnNewButtonClicked()
    {
        if (_targetData.dialogueLines.Any())
        {
            int option = EditorUtility.DisplayDialogComplex(
                "새로운 DialogueLine", "기존 데이터를 지우고 새로 시작하시겠습니까?",
                "확인", "취소", "Export 후 초기화"
            );
            switch (option)
            {
    ...
    private void OnExportButtonClicked()
    {
        ...
        try
        {
            ...
            Debug.Log($"DialogueLine 데이터가 성공적으로 Export 되었습니다: {path}");
        }
        catch (Exception e)
        {
    ...
    {
        // [수정] Undo.RegisterCompleteObjectUndo로 변경하여 전체 상태를 기록
    ...
    private void OnImportButtonClicked()
    {
        ...
        // [수정] Undo.RegisterCompleteObjectUndo로 변경
    ...
    {
        // OnEnable에 이미 등록되어 있으므로, 이 메서드가 Undo/Redo 시 호출됩니다.
        RefreshListView();
    }
